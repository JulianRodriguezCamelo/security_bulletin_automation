    ...
    def format_report(self, correlations: dict) -> str:
        ...
        for cve_id, result in correlations.items():
            if result["impacted_urls"]:
                ...
                lines.append(f"   {cve_id} → Sin tecnologías web afectadas en el inventario")

        if not any_impact:
            lines.append("  Ningún activo web del inventario usa tecnologías afectadas por este boletín.")
