...
class TenableClient:
    ...
    def get_asset_urls(self, max_assets: int = 200) -> list[str]:
        """Return a list of HTTPS URLs for web-facing assets in the Tenable inventory.

        Uses /workbenches/assets and builds URLs from FQDNs / hostnames.
        """
        url = f"{self.BASE_URL}/workbenches/assets"
        params = {"date_range": self.DATE_RANGE, "limit": max_assets}
        try:
            r = requests.get(url, headers=self.headers, params=params, timeout=20)
            if r.status_code != 200:
                logger.warning(f"[Tenable] /workbenches/assets returned HTTP {r.status_code}")
                return []

            urls: list[str] = []
            for asset in r.json().get("assets", []):
                # Prefer FQDN, fall back to hostname
            ...
            logger.error(f"[Tenable] Error fetching assets: {exc}")
            return []
