...
export default function Login({ onLogin }: Props) {
  ...
  const handleTotp = async (e: FormEvent) => {
    ...
    try {
      ...
      if (res.step === 'done') onLogin(res.username ?? username, res.role)
        ...
        <div className="card p-8">
          ...
          {step === 'totp_setup' && (
            <>
              ...
              <p className="text-xs text-[#A07080] mb-4">Escanea el QR con <strong>Microsoft Authenticator</strong>: abre la app → "+" → "Otra cuenta (Google, Facebook…)" → escanea el código.</p>
              {qr && (
          ...
          {step === 'totp' && (
            <>
              ...
              <p className="text-xs text-[#A07080] mb-4">Ingresa el código de 6 dígitos de <strong>Microsoft Authenticator</strong></p>
              <form onSubmit={handleTotp} className="space-y-4">
                <input className="input text-center tracking-widest text-lg" value={code} onChange={e => setCode(e.target.value)} placeholder="000 000" maxLength={6} required autoFocus />
                {error && <p className="text-xs text-red-600 font-medium">{error}</p>}
                <button className="btn-primary w-full" disabled={loading}>
